...
//_webpack.default.Dependency  Dependency基类
...
class CssModule extends _webpack.default.Module {
  constructor(dependency) {
    super(MODULE_TYPE, dependency.context);   //  Module接受 type 和 context;
...
class CssModuleFactory {
  create({ dependencies: [dependency] }, callback) {
    ...
    // if( /Common/.test(dependency.context) ){
    //   dependency.content = "";
    // }
...
class MiniCssExtractPlugin {
  ...
  apply(compiler) {
    ...
    compiler.hooks.thisCompilation.tap(pluginName, compilation => {
      ...
      // Dependecy 的每一个派生类在使用前，都会先确定对应的工厂对象
      ...
      */
      ...
      /**
       * 
       * mainTemplate 和  chunkTemplate 在  updateHashForChunk() 中触发
       *  
       * 
       * //在创建module和chunk内容前生成hash
       * 	updateHashForChunk(hash, chunk, moduleTemplate, dependencyTemplates) {
          ...
          compilaition -> seal() -> createHash() -> updateHashForChunk()
              seal(){
                ...
                  this.hooks.beforeHash.call();
                  this.createHash();   ->  this.hooks.hash.call(hash);
                  ... 
                    			template.updateHashForChunk(   -> Template.hooks.hashForChunk
                            chunkHash,
                            chunk,
                          ...
                          this.hooks.contentHash.call(chunk); ->  hooks.contentHash
                  this.hooks.afterHash.call();
                ...
            }
       */

       // chunkFilename 和 fileName 两个参数做转换；
      compilation.mainTemplate.hooks.hashForChunk.tap(pluginName, (hash, chunk) => {
        ...
        if (REGEXP_CONTENTHASH.test(chunkFilename))  { // [contenthash]
          hash.update(JSON.stringify(chunk.getChunkMaps(true).contentHash[MODULE_TYPE] || {}));
        }

        if (REGEXP_NAME.test(chunkFilename)) { // [name]
      ...
      /*
        ...
        chunk -> contentHash,modulesIterable
      ...
      */
      ...
      mainTemplate.hooks.localVars.tap(pluginName, (source, chunk) => {
        ...
        if (Object.keys(chunkMap).length > 0) {
          return Template.asString([
            source, 
            '', 
            '// object to store loaded CSS chunks', 'var installedCssChunks = {', 
      ...
      mainTemplate.hooks.requireEnsure.tap(pluginName, (source, chunk, hash) => {
        ...
        if (Object.keys(chunkMap).length > 0) {
          ...
          // /^Common/.test(chunk.name)?this.options.commonChunkFilename:
          const filenameTemplate =  this.options.chunkFilename;
          console.log( filenameTemplate )
          
          /**
           * 
           * hooks.assetPath
           * webpack\lib\TemplatedPathPlugin.js  replacePathVariables()
           * 
           * ""static/css/" + ({"1":"Common_test"}[chunkId]||chunkId) + "." + {"1":"842be006"}[chunkId] + ".chunk.css""
           */
          ...
          return Template.asString([
            ...
            Template.indent([
              ...
              Template.indent([`var href = ${linkHrefPathTransition};`, `var fullhref = ${mainTemplate.requireFn}.p + href;`, 
