...
// loader 如果某个 loader 在 pitch 方法中给出一个结果，那么这个过程会回过身来，并跳过剩下的 loader。
function pitch(request) {
  ...
  /**
   * 
   * 子编译器跟父编译器的一个差别在于，子编译器并没有完整的构建流程。 
   * 比如子编译器没有文件生成阶段( emit任务点)，它的文件生成必须挂靠在父编译器下面来实现。
   * 
   * 子编译器仍然拥有完整的模块解析和chunk生成阶段。
   * 也就是说我们可以利用子编译器来独立（于父编译器）跑完一个核心构建流程，
   * 额外生成一些需要的模块或者chunk。
   * 
   * 
   */
